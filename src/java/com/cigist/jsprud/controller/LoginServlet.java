...
import com.cigist.jsprud.impl.LoginImpl;
...
public class LoginServlet extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        synchronized (session) {
            accessCount = (Integer) session.getAttribute("username");
            if (accessCount == null) {
                accessCount = 0;   // autobox int to Integer
            } else {
                accessCount = new Integer(accessCount + 1);
            }
            session.setAttribute("username", accessCount);
        }

        System.out.println("username" + accessCount);
    }
